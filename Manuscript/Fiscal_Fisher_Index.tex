...
Preliminary signs of {\em imperfect} substitutability in the Treasury market would include a yield-to-maturity (YTM) spread between assets that are virtually identical in everything but name.
...
Using monthly CRSP data at the CUSIP level, Figure \ref{fig:YTM_Spread} shows a persistent gap in the median yields to maturity of Treasury notes and bonds with roughly the same time to maturity for the 1970--2020 period.
...
Considering the same data at a more granular level, I match Treasury securities by their days to maturity and compare their relative YTMs.
...
Potential matches incorporating Treasury bills are limited to those with less that six-months to maturity, ensuring that both securities have no other payments remaining until the maturity date.
...
% While the true aggregation function cannot be known, \citet{Christensen-Jorgenson-Lau:1971} show that the homogenous tanslog function 
...
% Additionally, \citet{Boisvert:1982} explains that the translog function is is more flexible than the Cobb-Douglas or constant elasticity of substitution (CES) functional forms by allowing the partial elastiticies of substituion between the assets to vary.
...
% These properties make the translog function the perfect starting point in deriving the best quantity index, without making overly stringent assumptions.
The preferred quantity index of \citet{Barnett:1980}, \citet{Barnett-Serletis:2000}, \citet{Belongia-Ireland:2014}, and others when aggregating monetary assets is the T\"{o}rnqvist-Theil Divisia quantity index 
\begin{equation*}
	\ln{M^d_t} = \ln{M^d_{t-1}} + \sum_{n=1}^{N}\frac{1}{2}(s_{n,t}+s_{n,t-1})\left(\ln{x^n_{t}} - \ln{x^n_{t-1}}\right).
	\label{DI}
\end{equation*}
Here, $M^d_t$ is the level of the Divisia quantity index at time $t$, $x^n_{t}$ is the nominal value of asset $n$, the weight on the marginal change in financial asset $n$ is expressed as
\begin{equation*}
	s_{n,t}  = \frac{\eta^n_{t}x^n_{t}}{\boldsymbol \eta_t' \mathbf{x}_t},
\end{equation*}
where $\mathbf{x}_t$ and $\boldsymbol \eta_t$ are $N \times 1$ vectors of nominal quantities and user costs, respectively.
% This type of quantity index is ubiquitous in the monetary aggregation literature.\footnote{See \citet{Barnett:1980}, \citet{Barnett-Serletis:2000}, and \citet{Belongia-Ireland:2014} for a preview of the monetary aggregation literature.}

While highly efficient in terms of its a precision and ease of calculation, one drawback of the T\"{o}rnqvist-Theil Divisia index is that it does not handle assets coming into and out of existence ($x^n_{t} = 0$ for some $n,t$) very well.\footnote{
	Another important strength of the Divisia functional form is that it simply looks like a weighted average, especially in its continuous form, which makes it easier to interpret in application or policy discussions.
}
...
Under these considerations, I find that a Fisher ideal index is optimal for the remaining analysis.
...
This model incorporates both short- and long-term debt issued by the government, as well as an alternative long-term asset that acts as a pure savings vehicle and can be used as the benchmark security. 
...
A portion $\alpha \in (0,1)$ of the previous period's stock of long-term bonds mature, making the maturity of these bonds $\sfrac{1}{\alpha}$ periods.
Thus, the stock of outstanding long-term debt evolves such that 
\begin{equation}
	B^L_t = (1-\alpha)B^L_{t-1} + B^{L,n}_t.
	\label{eq:LoM_DebtLevel}
\end{equation}
The average nominal interest rate paid on the stock of outstanding long-term debt $r^L_t$ progresses such that
\begin{equation}
...
\begin{equation}
	...
	\label{eq:HH_MonServices}
\end{equation}
where $\theta_s + \theta_L = 1$ are the linear weights on the short- and long-term bonds, respectively, $\omega_{j,k}$ is the partial elasticity of substitution between assets $j$ and $k$, $\omega_{j,k}=\omega_{k,j}$ for all $j,k$, and $\sum_{k} \omega_{j,k} = 0$ for $j,k={s,L}$.
The use of a translog function stems from the mathematical derivation of the quanitity index itself, which requires a twice-continuously-differentiable linear homogenous aggregator as a starting point.
Appendix \ref{app:Translog} explains in more detail why the translog function is preferred in this baseline situation, and why a constant-elasticity-of-substitution (CES) function can be considered in later analyses without loss of generality.

\paragraph{Bonds in the Utility Function}
...
For instance, bonds might be seen more as a savings tool than bills, but they are still typically viewed as more liquid than other (non-Treasury) debt securities available. 
Bills, though largely influenced by their inherent liquidity, are also widely recognized as a safe asset, so their inherent qualities aren't necessarily separable. 
...
Second, if liquidity/safety were explicitly modeled in a comprehensible/tractable manner, the outcome would remain consistent. 
Generally, it is evident that when transaction costs or cash-in-advance components are added to the budget constraint, there exists a representation with money/bonds in the utility function which can yield the same first-order conditions. 
The main aim of this study is to construct a nonparametric quantity index monitoring US Treasury debt. 
As illustrated in Appendix \ref{app:usercost_derivation}, this entire exercise hinges the equivalency between marginal benefit and cost. 
...
This underscores the need for agent optimization in the creation of a well-defined index number, enabling its derivation even without an explicit understanding of the aggregation function.
...
\section{Results}
\label{sec:Results}
In this section I present the measurement derived above in various forms as well as attempt to provide evidence of its validity by analyzing the more dramatic shifts in the measurement and the historical context behind them.
\subsection{The Measure}
\label{subsec:Results_Measure}
The Fisher ideal quantity index of US fiscal debt---and its simple-sum counterpart---is presented in Figure \ref{fig:MS_Index}.
...
This series is then used to constuct a quantity index for the monetary services, separated from the quantity of the underlying principal value.\footnote{
	These month-over-month growth rates are rather noisy and can be difficult to interpret.
	The values are therefore relegated to Figure \ref{fig:Growth_Spread} in Appendix \ref{app:initial_results}.
}
...
\begin{figure}[p]
	...
	\label{fig:MS_Index}
\end{figure}
The quantity index is shown in Figure \ref{fig:MS_Index}, while the year-over-year growth rate of the index is shown in Figure \ref{fig:YoY_Growth}.
...
\label{subsec:Results_Events}
Just as with other financial assets, the monetary services of US Treasuries can ebb and flow with economic environment. 
In this section, I highlight some of the more notable events in the sample period and how they relate to the growth of these monetary services.
To do so, I decompose the month-over-month growth rates of the monetary services index by the type of Treasury security to analyze their respective contributions.

\subsubsection{1987--1991}
This section examines the late 1980s and early 1990s, a period marked by significant economic fluctuations globally. 
Key events during this time include the Savings and Loan Crisis, the collapse of the Berlin Wall, the Iraqi invasion of Kuwait, the onset of the first Gulf War, and the conclusion of the Cold War. 
These events likely spurred a heightened demand for market safety. 
Additionally, in November 1988, a pivotal change occurred when Congress removed the fixed 4.25\% interest rate cap on Treasury bonds, enabling their rates to vary and facilitating their issuance as required.

The impact of these developments on the role of bonds in the market is evident. 
Figure \ref{fig:Contribution_1987-1991} distinctly showcases the contribution of bonds, separate from other securities. 
Beginning in June 1988, bonds significantly influenced the growth rate of fiscal monetary services. 
A notable spike in their influence is observed in November 1989, coinciding with the fall of the Berlin Wall, with an increase exceeding six percentage points. 
This upward trend in bond services persisted until June 1991, aligning with the dissolution of the Soviet-led Council for Mutual Economic Assistance (Comecon), a precursor to the Soviet Union's disintegration in December 1991. 
Given all that happened during this period, the dramatic rise in the monetary services of bonds---and Treasuries, more generally, as seen in Figure \ref{fig:YoY_Growth}---is understandable.
\begin{figure}[h]
	\centering
	\includegraphics[width=0.8\textwidth]{../Figures/Contribution_MS_1987-1991.jpg}
	\caption{Contribution of Bonds to Month-over-Month Monetary Services Growth, 1987--1991}
	\label{fig:Contribution_1987-1991}
\end{figure}

\subsubsection{2007--2009}
This section delves into the Great Recession, spanning from December 2007 to June 2009, with a specific focus on the final quarter of 2008. 
This period was characterized by heightened market volatility and uncertainty, as reflected in the notable decline in monetary services provided by U.S.\ Treasuries seen in Figures \ref{fig:MS_Index} and \ref{fig:YoY_Growth}. 
This decline raises the question: what led to this abrupt reduction in monetary services?

\citet[pg 302]{Menand-Younger:2023} provide insights into the repurchase agreement (repo) market's dynamics during this period. 
They note that in early 2008, major stand-alone broker-dealers experienced significant changes. 
Bear Sterns and Merrill Lynch were aquired by commercial banks J.P.\ Morgan Chase and Bank of America, respectively, Lehman Brothers collapsed, and Goldman Sachs and Morgan Stanley reorganized as bank holding companies. 
These developments resulted in a dramatic reduction in market intermediation capacity, as evidenced by the decrease in gross traded volumes despite market expansion.
\begin{figure}[h!]
	\centering
	\includegraphics[width=0.8\textwidth]{../Figures/Contribution_MS_2007-2009.jpg}
	\caption{Contribution of Bills to Month-Over-Month Monetary Services Growth, 2007--2009}
	\label{fig:Contribution_2007-2009}
\end{figure}

This shift resulted in a new, more stringent regulatory environment for the majority of the market, which drastically reduced liquidity in the repo market and Treasury market, more generally. 
Figure \ref{fig:Contribution_2007-2009} corroborates this, indicating that typically highly-liquid bills primarily drove the dramatic downturn in the monetary services of fiscal debt between September, 2008, and January, 2009.
...
\section{Translog Functions and Index Number Theory}
\label{app:Translog}
...
Additionally, \citet{Boisvert:1982} explains that the translog function is is more flexible than the Cobb-Douglas or constant elasticity of substitution (CES) functional forms by allowing the partial elastiticies of substituion between the assets to vary.
He also goes on to explain that this functional form can be viewed as a) an exact aggregator function, b) a second-order Taylor approximation to a general, but unknown aggregator function, or c) a second-order approximation to a CES function.
These properties make the translog function the perfect starting point in deriving user cost and quantity index, without making overly stringent assumptions.
Additionally, \citet{Diewert:1976} shows that ideal indexes are derived from quadratic mean of order 2 quantity indexes, which corresponds to the assumed translog aggregator function in Section \ref{sec:Theory} and is even recommended as the preferred index to use \citep[][Section 5]{Diewert:1976}.
