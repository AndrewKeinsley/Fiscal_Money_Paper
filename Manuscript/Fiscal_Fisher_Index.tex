...
I also control for the spread in their respective coupon rates, a general time to maturity (months/years), the difference in their days to maturity, and a calendar year dummy.\footnote{
	Note that the coupon rate for bills is zero.
	Since the matching threshold is one day, the ``Days-to-Maturity Difference'' variable only takes values of -1, 0, and 1. 
	Standard errors are heteroskedasitcally robust (HC1).
}

Table \ref{tab:Relative_Liquidity} presents the results for both the notes--bills and bonds--notes, analyses.

In each case, I find that a one percentage point increase in the spread between the assets' bid-ask ratios 
...
\begin{table}[h]
	...
	\begin{threeparttable}
		\begin{tabular}{l d{2.4} d{2.4} } \toprule
										& \multicolumn{1}{c}{Notes--Bills$^{b}$}  & \multicolumn{1}{c}{Bonds--Notes} 	\\ \midrule
			Relative Bid-Ask Spread 	& -1.6034^{***}					& -1.6568^{***}					\\ [-0.5em]
										& (0.255)							& (0.081)							\\
			Days-to-Maturity Difference & 0.0156^{***}						& 0.0000^{***}						\\ [-0.5em]
										& (0.005)							& (0.000)							\\
			Coupon Rate Spread 			& 0.0236^{***}						& 0.0472^{***}						\\ [-0.5em]
										& (0.002)							& (0.004)							\\
			Months to Maturity 			& -0.0220^{***} 					& 									\\ [-0.5em]
										& (0.003)							&									\\
			Years to Maturity 			& 									& -0.0172^{***}						\\ [-0.5em]
										& 									& (0.002)							\\
			Year Dummy 					& 0.0027^{***}						& -0.0079^{***}						\\ [-0.5em]
										& (0.001) 							& (0.001)							\\ 
			Constant					& -5.2439^{***} 					& 15.6621^{***}						\\ [-0.5em]
										& (1.081)							& (1.056) 							\\\midrule
			Observations				& \multicolumn{1}{c}{3966}			& \multicolumn{1}{c}{7430}			\\
			R-Squared					& 0.096								& 0.505								\\
			F-statistic 				& 39.14								& 207.2								\\ \bottomrule 
		\end{tabular}
		\begin{tablenotes}
			\item[a] \footnotesize{The dependent variable is the yield-to-maturity spread between securities matched by thier respective days to maturity. Standard errors are heteroskedasticity robust (HC1)}
			\item[b] \footnotesize{The notes--bills analysis is limited to those securities with six months to maturity or less. This ensures that both securities have no other payments remaining until the maturity date.}
		\end{tablenotes}
	\end{threeparttable}
\end{table}
