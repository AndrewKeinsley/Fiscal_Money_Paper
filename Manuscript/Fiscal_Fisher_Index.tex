...
\label{subsec:MktSeg}
Fiscal debt is ubiquitously measured as the sum of outstanding principal values.
Simple-sum aggregates come with an assumption that the underlying assets are perfect substitutes, such that they have equal weights. 
...
The results of the bonds--notes and bonds--bills analyses paint bonds as primarily a savings vehicle in financial markets. 
...
Here, $M^d_t$ is the level of the Divisia monetary aggregate, $m_{i,t}$ is the nominal value of asset $i$, and 
\begin{equation*}
	s_{n,t}  = \frac{\eta_{n,t}x_{n,t}}{\boldsymbol \eta_t \mathbf{x'}_t}
\end{equation*}
...
One drawback of the T\"{o}rnqvist-Theil Divisia index is that it does not handle assets coming into and out of the market ($x_{n,t} = 0$ for some $t$) very well.
...
A common solution to this problem is to either temporarily or completely move to a Fisher ideal index:
\begin{equation}
...
Additionally, \citet{Diewert:1976} shows that the ideal index a derivation from a quadratic mean of order 2 quantity index, which corresponds to the assumed translog aggregator function and is even recommended as the preferred index to use \citep[][Section 5]{Diewert:1976}.
Because of these useful properties and the nature of the data considered, an ideal index is used in the remaining analysis.
